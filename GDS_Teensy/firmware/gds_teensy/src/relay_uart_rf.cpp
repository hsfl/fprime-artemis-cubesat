...
void RelayUartRf::poll() {
  ...
  if (!m_lastRadioReady) {
    // Bytes received while the SDN/POR/init sequence ran are ambiguous.
    discardRadioWorkOnOff();
  ...
  if (!m_rf.isReady()) {
    m_lastRadioReady = false;
    // Keep completed radio-to-USB downlinks, but purge uncertain RF work.
    discardRadioWorkOnOff();
  }
  serviceDownlinkQueue();
}
...
void RelayUartRf::processRfSegment(const uint8_t* packet, uint8_t packetLen) {
  ...
  // Re-ACK a retry of a completed message, but never forward it twice.
...
void RelayUartRf::emitLinkStatus() {
  ...
  const int n =
      snprintf(statusLine,
               sizeof(statusLine),
               "#LINK_STATUS uart_rx=%lu uart_tx=%lu rf_rx_pkt=%lu rf_tx_pkt=%lu rf_rx_msg=%lu rf_tx_msg=%lu rf_rx_seg=%lu rf_tx_seg=%lu crc_drops=%lu framing_drops=%lu uart_timeouts=%lu rf_reasm_timeouts=%lu rf_reasm_drops=%lu rf_oversize_drops=%lu rf_tx_drops=%lu rf_tx_timeouts=%lu rf_recoveries=%lu rf_tx_terminal_failures=%lu rf_msg_id_gaps=%lu rf_ack_rx=%lu rf_ack_tx=%lu rf_retries=%lu rf_ack_timeouts=%lu rf_wrong_network=%lu rf_wrong_address=%lu rf_wrong_version=%lu rf_duplicate_drops=%lu rf_recovery_purged_uplinks=%lu rf_recovery_discarded_bytes=%lu up_q_drops=%lu down_q_drops=%lu\\n",
