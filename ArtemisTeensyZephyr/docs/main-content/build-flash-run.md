...
1. In order to build the ZephyrDeployment application, or any other F´ application, we first need to generate a build directory. This can be done with the following commands:
...
2. The next step is to build the ZephyrDeployment application's code.
```sh
# In fprime-zephyr-reference (fprime-venv)
fprime-util build
```
...
```sh
# In fprime-zephyr-reference (fprime-venv)
fprime-gds -n --dictionary ./build-artifacts/zephyr/fprime-zephyr-deployment/dict/ZephyrDeploymentTopologyDictionary.json --communication-selection uart --uart-baud 115200 --output-unframed-data

# Or
fprime-gds -n --dictionary ./build-artifacts/zephyr/fprime-zephyr-deployment/dict/ZephyrDeploymentTopologyDictionary.json --communication-selection uart --uart-device /dev/cu.usbmodem142101 --uart-baud 115200 
