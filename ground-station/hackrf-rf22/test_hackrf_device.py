...
class HackRFDeviceTests(unittest.TestCase):
    def test_rf_amp_and_antenna_power_are_hard_disabled(self) -> None:
        for config in (
            HackRFConfig(amp_enabled=True),
            HackRFConfig(antenna_power_enabled=True),
        ):
    ...
    def test_constructor_is_usb_idle_and_open_applies_c3m_defaults(self) -> None:
        ...
        self.assertEqual(values["hackrf_set_lna_gain"], 8)
