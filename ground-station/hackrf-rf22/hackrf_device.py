...
class HackRFConfig:
    """C3M defaults; the RF amplifier starts off and bias power stays off."""
...
class HackRFDevice:
    ...
    def set_rx_gains(self, lna_gain_db: int, vga_gain_db: int) -> None:
        """Apply validated RX gains while idle or receiving.

        libhackrf supports changing both receive gain stages during RX.  The
        bridge resets its decoder and discards queued samples around this call
        so samples from two gain settings are never scored in one window.
        """

        if lna_gain_db not in range(0, 41, 8):
            raise ValueError("lna_gain_db must be 0..40 dB in 8 dB steps")
        if vga_gain_db not in range(0, 63, 2):
            raise ValueError("vga_gain_db must be 0..62 dB in 2 dB steps")
        with self._mode_lock:
            self._require_open()
            if self._mode not in ("idle", "rx"):
                raise HackRFStateError(
                    f"cannot change RX gains while device mode is {self._mode}"
                )
            self._call("hackrf_set_lna_gain", self._device, lna_gain_db)
            self._call("hackrf_set_vga_gain", self._device, vga_gain_db)

    def set_tx_gain(self, tx_gain_db: int) -> None:
        """Apply a validated TX VGA gain while idle or receiving."""

        if not 0 <= tx_gain_db <= 47:
            raise ValueError("tx_gain_db must be 0..47 dB")
        with self._mode_lock:
            self._require_open()
            if self._mode not in ("idle", "rx"):
                raise HackRFStateError(
                    f"cannot change TX gain while device mode is {self._mode}"
                )
            self._call("hackrf_set_txvga_gain", self._device, tx_gain_db)

    def set_amp_enabled(self, enabled: bool) -> None:
        """Set HackRF's shared RF amplifier while the radio is idle."""
        ...
        with self._mode_lock:
            ...
            if self._mode != "idle":
                raise HackRFStateError(
                    f"cannot change RF amplifier while device mode is {self._mode}"
                )
            self._call("hackrf_set_amp_enable", self._device, int(enabled))
