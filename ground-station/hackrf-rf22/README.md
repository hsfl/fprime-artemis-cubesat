...
BLUF: `./tools/c3m-sdr` is the plug-and-play C3M operator entry point. It
identifies the known CRC-valid C3M RF22 downlink, automatically selects RX and
TX gains, freezes them for the session, then starts F Prime GDS and the
payload/livestream UI. Operators do not tune RF parameters.
...
| Session behavior | calibrate once at startup, then freeze gains |
...
Moving outside does not require operator gain flags: restart `c3m-sdr` at the
new geometry and let it calibrate again. The RF amplifier is not enabled just
because the geometry changed: normal gains must fail first. Outdoor mission readiness still
requires end-to-end smoke/livestream/science proof; automatic gain selection is
not itself outdoor qualification or regulatory authorization.
...
Each run records its calibration candidates and selection in:
...
`/tmp/c3m-sdr/latest` points to the newest run. The proof collector accepts a
completed automatic calibration only when the selected RX window passed and
the selected TX probe contains an exact Pong.
