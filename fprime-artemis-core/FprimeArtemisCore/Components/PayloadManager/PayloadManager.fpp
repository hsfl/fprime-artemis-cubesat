module Components {
    @ Payload manager exposing stable mission-facing behavior.
    active component PayloadManager {
        ...
        @ Collection request input from ScienceApp
        sync input port requestIn: Components.PayloadCaptureRequest

        @ Payload readiness from the active driver
        sync input port driverStateIn: Components.PayloadStateReport

        @ Driver status input
        sync input port driverStatusIn: Components.ScienceProductDescriptor

        @ Driver request output
        output port driverRequestOut: Components.PayloadCaptureRequest

        @ Payload status output to ScienceApp
        ...
        @ Payload readiness toward the flight controller, forwarded from the
        @ driver. Connect to FlightControllerLinkManager.payloadStateIn.
        output port stateOut: Components.PayloadStateReport

        @ Request latest payload status from driver
        ...
        @ Whether the payload can take a capture now
        telemetry PayloadState: Components.PayloadState update on change

        @ Payload request forwarded to driver
