#include "FprimeArtemisCore/Components/PayloadManager/PayloadManager.hpp"
...
PayloadManager::PayloadManager(const char* const compName)
    : PayloadManagerComponentBase(compName),
      ...
      m_payloadState(Components::PayloadState::UNKNOWN) {}
...
void PayloadManager::driverStateIn_handler(FwIndexType portNum, const Components::PayloadState& payloadState) {
    static_cast<void>(portNum);
    ...
    if (this->isConnected_stateOut_OutputPort(0)) {
        this->stateOut_out(0, payloadState);
    }
}
