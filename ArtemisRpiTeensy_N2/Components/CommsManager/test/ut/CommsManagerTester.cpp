...
CommsManagerTester::CommsManagerTester()
    : CommsManagerGTestBase("CommsManagerTester", MAX_HISTORY_SIZE),
      component("CommsManager") {
    this->initComponents();
    this->connectPorts();
}
...
void CommsManagerTester::testRejectsDownlinkWithoutScience() {
    this->clearHistory();
    ...
    ASSERT_CMD_RESPONSE(0, CommsManagerComponentBase::OPCODE_REQUEST_SCIENCE_DOWNLINK, 0,
...
void CommsManagerTester::testRequestsScienceDownlinkAndCompletionClearsState() {
    this->clearHistory();
    ...
    ASSERT_CMD_RESPONSE(1, CommsManagerComponentBase::OPCODE_REQUEST_SCIENCE_DOWNLINK, 1,
                        Fw::CmdResponse::VALIDATION_ERROR);
}

void CommsManagerTester::testDownlinkFailureReturnsBase() {
    this->clearHistory();
...
void CommsManagerTester::testIgnoresPayloadStatusWhenInactive() {
    this->clearHistory();
...
void CommsManagerTester::testAdapterStatusPollingAndRssiPing() {
    this->clearHistory();

    this->sendCmd_REQUEST_LINK_STATUS(0, 0);
    this->component.doDispatch();
    ASSERT_CMD_RESPONSE_SIZE(1);
    ASSERT_from_adapterRequestOut_SIZE(1);
    ASSERT_from_adapterRequestOut(0, 1);

    this->invoke_to_adapterStatusIn(0, 2);
    this->component.doDispatch();
    ...
    this->component.doDispatch();
    ASSERT_CMD_RESPONSE_SIZE(2);
    ASSERT_from_adapterRequestOut_SIZE(2);
    ASSERT_from_adapterRequestOut(1, 3);

    this->invoke_to_adapterStatusIn(0, 3);
    this->component.doDispatch();
...
void CommsManagerTester::testRunPublishesHealthFromLinkState() {
    this->clearHistory();
