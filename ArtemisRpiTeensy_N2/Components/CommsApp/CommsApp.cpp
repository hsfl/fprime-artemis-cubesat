...
void CommsApp::driverStatusIn_handler(FwIndexType portNum,
    ...
    this->updateCompatibilityLinkState();
...
void CommsApp::scheduleRecovery() {
    // The app runs every two seconds: 15, 60, and 450 ticks are 30 s, 120 s,
    // and 900 s. The final interval remains capped for subsequent attempts.
...
void CommsApp::emitRadioHealth() {
    ...
    if (this->m_radioStatusKnown && this->m_radioState == Components::RadioState::READY &&
        ...
        // A radio-local fault degrades comms without asking Health to reset the
        // responsive Pi or Teensy application component.
...
void CommsApp::updateCompatibilityLinkState() {
    const U32 previousLinkState = this->m_linkState;
    if (!this->m_radioStatusKnown) {
        this->m_linkState = 0U;
    } else if (this->m_radioState == Components::RadioState::OFF) {
        this->m_linkState = this->m_desiredRadioEnabled ? 1U : 0U;
    } else if (this->m_radioFault == Components::RadioFault::NONE) {
        this->m_linkState = 2U;
    } else {
        this->m_linkState = 3U;
    }
