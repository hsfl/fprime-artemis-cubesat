#include "Components/GpsDriver_Artemis/GpsDriver_Artemis.hpp"

namespace Components {

GpsDriver_Artemis::GpsDriver_Artemis(const char* const compName)
    : GpsDriver_ArtemisComponentBase(compName),
      m_lastRequestKey(0),
      m_requestCount(0),
...
GpsDriver_Artemis::~GpsDriver_Artemis() {}

void GpsDriver_Artemis::pingIn_handler(FwIndexType portNum, U32 key) {
    static_cast<void>(portNum);
    this->pingOut_out(0, key);
}

void GpsDriver_Artemis::requestIn_handler(FwIndexType portNum, U32 key) {
    static_cast<void>(portNum);
...
void GpsDriver_Artemis::updateFixModel() {
...
U32 GpsDriver_Artemis::toStatusKey() const {
