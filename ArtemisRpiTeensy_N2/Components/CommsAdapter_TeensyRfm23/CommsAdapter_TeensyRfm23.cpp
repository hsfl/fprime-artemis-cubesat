...
void CommsAdapter_TeensyRfm23::teensyResponseIn_handler(FwIndexType portNum, Fw::Buffer& fwBuffer) {
    static_cast<void>(portNum);
    const LinkState previousState = this->m_linkState;
    bool parsed = false;
    if (fwBuffer.isValid()) {
...
void CommsAdapter_TeensyRfm23::emitStatus() {
    const U32 linkStatus = this->toStatusKey();
...
bool CommsAdapter_TeensyRfm23::sendRfStatsRequest() {
...
bool CommsAdapter_TeensyRfm23::parseRfStatsResponse(const U8* data, FwSizeType size) {
...
U16 CommsAdapter_TeensyRfm23::readLe16(const U8* data) {
    return static_cast<U16>(data[0]) | static_cast<U16>(data[1] << 8U);
}

U32 CommsAdapter_TeensyRfm23::readLe32(const U8* data) {
